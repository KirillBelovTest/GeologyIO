...
(* :Title: SEGY *)
(* :Context: GeologyIO`SEGY` *)
(* :Version: 0.2.1 *)
(* :Keywords: Geology; SEG-Y; *)
...
fromEBCDICCharacterCode[bytes: {__Integer}] := 
	StringJoin[(bytes /. $EBCDICCharacters) //. {_Integer :> "?"}]


toEBCDICCharacterCode[text_String] := 
	(StringSplit[text, ""] /. ($EBCDICCharacters)[[All, {2, 1}]]) /. {_String :> 74}
...
toSEGYBinaryHeader[rules: {(_String -> _Integer)..}] /; 
	...
	Module[{
		...
		Table[
			...
			bytes[[position]] = Reverse[ImportString[ExportString[name /. rules, type], "Byte"]], 
			{header, $SEGYBinaryHeaderSpetification}
		]; 
...
createSEGYBinaryHeader[{}, OptionsPattern[]] := 
	Module[{
		bytes = ConstantArray[0, 400], 
		samplesFormatCodeBytes = 
		IntegerDigits[If[# === Automatic, 1, #], 256, 2]&[OptionValue["SamplesFormatCode"]], 
		numberDataTracesBytes = 
		IntegerDigits[If[# === Automatic, 0, #], 256, 2]&[OptionValue["NumberDataTraces"]], 
		numberOfSamplesForReelBytes = 
		IntegerDigits[If[# === Automatic, 0, #], 256, 2]&[OptionValue["NumberOfSamplesForReel"]], 
		intervalReelRecord = 
		IntegerDigits[1000, 256, 2], 
		measurementSystem = 
		IntegerDigits[2, 256, 2]
...
$SEGYTraceHeadersKeys = 
	{"tracl","tracr","fldr","tracf","ep","cdp","cdpt","trid","nvs","nhs","duse",
	"offset","gelev","selev","sdepth","gdel","sdel","swdep","gwdep","scalel",
	"scalco","sx","sy","gx","gy","counit","wevel","swevel","sut","gut","sstat",
	"gstat","tstat","laga","lagb","delrt","muts","mute","ns","dt","gain","igc",
	"igi","corr","sfs","sfe","slen","styp","stas","stae","tatyp","afilf","afils",
	"nofilf","nofils","lcf","hcf","lcs","hcs","year","day","hour","minute","sec",
	"timbas","trwf","grnors","grnofr","grnlof","gaps","ofrav","cdpx","cdpy","iline",
	"xline","spnum","spscal","trunit"}


$SEGYTraceHeaderSpetification = 
	{{1,4,1},{5,8,1},{9,12,1},{13,16,1},{17,20,1},{21,24,1},{25,28,1},
	{29,30,-1},{31,32,1},{33,34,1},{35,36,1},{37,40,-1},{41,44,-1},
	{45,48,-1},{49,52,-1},{53,56,-1},{57,60,-1},{61,64,-1},{65,68,-1},
	{69,70,-1},{71,72,-1},{73,76,-1},{77,80,-1},{81,84,-1},{85,88,-1},
	{89,90,1},{91,92,1},{93,94,1},{95,96,-1},{97,98,-1},{99,100,-1},
	{101,102,-1},{103,104,-1},{105,106,-1},{107,108,-1},{109,110,-1},
	{111,112,-1},{113,114,-1},{115,116,1},{117,118,1},{119,120,1},
	{121,122,-1},{123,124,-1},{125,126,1},{127,128,1},{129,130,1},
	{131,134,1},{133,134,1},{135,136,1},{137,138,1},{139,140,1},
	{141,142,1},{143,144,1},{145,146,1},{147,148,1},{149,150,1},{151,152,1},
	{153,154,1},{155,156,1},{157,158,1},{159,160,1},{161,162,1},{163,164,1},
	{165,166,1},{167,168,1},{169,170,1},{171,172,1},{173,174,1},{175,176,1},
	{177,178,1},{179,180,1},{181,184,-1},{185,188,-1},{189,192,-1},
	{193,196,-1},{197,200,1},{201,202,-1},{203,204,-1}}
...
traceHeaderKeyQ[k_String] := 
	MemberQ[$SEGYTraceHeadersKeys, ToLowerCase[k]]
...
fromSEGYTraceHeader[target: "MVM" | "C"] := fromSEGYTraceHeader[target] = 
	With[{$SEGYTraceHeaderSpetification = $SEGYTraceHeaderSpetification}, 
		Compile[{
			{bytes, _Integer, 1}
		}, 
			Module[{j = 1, blist = {0}, len = 0, beg = 0, end = 0, type = 0, num1 = 0, 
			spec = $SEGYTraceHeaderSpetification}, 
				Table[
					...
					j * (num1 * 256^len + Sum[blist[[i + 1]] * 256^(len - i), {i, 1, len}]), 
					{param, spec}
				]
			]
		]
...
SEGYImport[file_String?FileExistsQ, OptionsPattern[]] /; 
	...
	Module[{
		...
		Which[
			...
			traceHeaders = ConstantArray[0, {numberDataTraces, Length[$SEGYTraceHeaderSpetification]}];
			...
			Return[
				SEGYData[{
					...
					"TraceHeaders" -> {$SEGYTraceHeadersKeys, traceHeaders}, 
					"Traces" -> traces
				}]
			], 
...
SEGYLoad[{"File" -> file_String?FileExistsQ, "Positions" -> positions: {__Integer}}, OptionsPattern[]] /; 
	...
	Module[{
		stream = OpenRead[file, BinaryFormat -> True], 
		compilationTarget = If[# === "MVM" || # === "C", #, "MVM"]&@OptionValue["CompilationTarget"], 
		traceHeaders = ConstantArray[0, {Length[positions], Length[$SEGYTraceHeadersKeys]}]
	}, 
		Table[
		...
		Switch[OptionValue["TraceHeadersForm"], 
			List, 
			Return[{$SEGYTraceHeadersKeys, traceHeaders}], 
			
			Rule, 
			Return[Map[MapThread[Rule, {$SEGYTraceHeadersKeys, #}]&, traceHeaders]], 
			
			Association, 
			Return[Association[MapIndexed[Rule[#2[[1]], #1]&, (Association /@ Map[MapThread[Rule, {$SEGYTraceHeadersKeys, #}]&, traceHeaders])]]], 
...
SEGYLoad[{"File" -> file_String?FileExistsQ, "Positions" -> position_Integer}, OptionsPattern[]] /; 
	...
	Module[{
		...
		Return[MapThread[Rule, {$SEGYTraceHeadersKeys, traceHeader}]]
	]
...
SEGYExport[file_String, segy_SEGYData?memorySEGYQ, OptionsPattern[]] /; 
	...
	Module[{
		...
		numberDataTraces = 
		If[# === Automatic, segy["BinaryHeader", "NumberDataTraces"], #]&@OptionValue["NumberDataTraces"]
	}, 
...
SEGYWrite[file_String?FileExistsQ, 
	{
		_?traceHeadersKeyQ -> (traceHeaders: {{__Integer}.. } ?(MatchQ[Dimensions[#], {_?Positive, Length[$SEGYTraceHeadersKeys]}]&)), 
...
SEGYWrite[file_String?FileExistsQ, traces: ({{__Real}.. } | {{__Integer}.. })?MatrixQ, opts: OptionsPattern[]] := 
	SEGYWrite[file, {
		"TraceHeaders" -> Developer`ToPackedArray[ConstantArray[0, {Length[traces], Length[$SEGYTraceHeadersKeys]}]], 
		"Traces" -> Developer`ToPackedArray[traces]
	}, opts]


SEGYWrite[file_String?FileExistsQ, {
	traceHeaders: {{(_String -> _Integer).. }.. } ?(MatchQ[ToLowerCase[#[[All, All, 1]]], {ToLowerCase[$SEGYTraceHeadersKeys].. }]&), 
...
SEGYWrite[file_String?FileExistsQ, trace: {__Real} | {__Integer}, opts: OptionsPattern[]] := 
	SEGYWrite[file, {
		"TraceHeaders" -> Developer`ToPackedArray[{ConstantArray[0, Length[$SEGYTraceHeadersKeys]]}], 
...
(segy_SEGYData?delayedSEGYQ)[key_String?traceHeadersUnloadedKeyQ, i_Integer?Positive] := 
	Block[{ud = segy[[1, 3, -1]]}, ud[[-1, -1]] = ud[[-1, -1, i]]; ud]


(segy_SEGYData?delayedSEGYQ)[key_String?traceHeadersUnloadedKeyQ, i: {__Integer?Positive} | Span[__]] := 
	Block[{ud = segy[[1, 3, -1]]}, ud[[-1, -1]] = ud[[-1, -1, i]]; ud]
...
SEGYData /: Set[name_Symbol, segy_SEGYData] := 
	Module[{}, 
		...
		name /: Set[name[_?traceHeadersKeyQ], newTraceHeadersValues: {{__Integer}.. }] /; 
			...
			Length[newTraceHeadersValues[[1]]] === Length[$SEGYTraceHeadersKeys] := 
		...
		name /: Set[name[_?traceHeadersKeyQ, i_Integer], newTraceHeaderValues: {__Integer}] /; 
			memorySEGYQ[name] && 
			i =!= 0 && 
			Length[newTraceHeaderValues] === Length[$SEGYTraceHeadersKeys] := 
		...
		name /: Set[name[_?traceHeadersKeyQ, i_Integer, key_String], newTraceHeaderValue_Integer] /; 
			memorySEGYQ[name] && 
			i =!= 0 && 
			MemberQ[ToLowerCase[$SEGYTraceHeadersKeys], ToLowerCase[key]] := 
			name[[1, 3, 2, 2, i, Position[ToLowerCase[$SEGYTraceHeadersKeys], ToLowerCase[key]][[1, 1]]]] = newTraceHeaderValue; 
		...
		name /: Set[name[_?traceHeadersKeyQ, i_Integer, keys: {__String}], newTraceHeadersValues: {__Integer}] /; 
			memorySEGYQ[name] && 
			i =!= 0 && 
			SubsetQ[ToLowerCase[$SEGYTraceHeadersKeys], ToLowerCase[keys]] := 
			name[[1, 3, 2, 2, i, Table[Position[ToLowerCase[$SEGYTraceHeadersKeys], ToLowerCase[key]][[1, 1]], {key, keys}]]] = newTraceHeadersValues; 
		...
		name /: Set[name[_?traceHeadersKeyQ, i: {__Integer} | _Span | All, key_String], newTraceHeadersValues: {__Integer}] /; 
			...
			MemberQ[ToLowerCase[$SEGYTraceHeadersKeys], ToLowerCase[key]] := 
			name[[1, 3, 2, 2, i, Position[ToLowerCase[$SEGYTraceHeadersKeys], ToLowerCase[key]][[1, 1]]]] = newTraceHeadersValues; 
...
$segyFormatIcon = 
	Hyperlink[
		Import[FileNameJoin[{DirectoryName[$InputFileName], "Resources", "SEGLogo.png"}]], 
